...
INGEST = IngestionPipeline(CONFIG, CHUNKER, EMBEDDER, STORE)
...
@mcp.tool()
def ingest_documents(path: str, category: str = "auto") -> str:
    if INGEST.model_needs_reindex():
        return _err("model_changed", "Embedding model differs from stored data; re-index required.")
    import os
    if os.path.isdir(path):
        results = INGEST.ingest_directory(path, category)
    else:
        results = [INGEST.ingest_file(path, category)]
...
@mcp.tool()
def search_knowledge(query: str, filters: dict | None = None, top_k: int = 10) -> str:
    try:
